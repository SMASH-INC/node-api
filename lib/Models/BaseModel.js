...
class BaseModel {
    ...
    /**
     * Adds an additional property to the model
        ...
        for (let iter = 0; iter < props.length; iter += 1) {
            if (typeof this[props[iter]] !== 'function') {
                ...
                if (value === 'additionalProperties') {
                    const addPropKeys = Object.keys(this[value]);
                    addPropKeys.forEach((key) => {
                        newDict[key] = this[value][key];
                    });
                } else if (this[props[iter]] instanceof Object &&
                    ...
                    this[props[iter]].forEach((item, index) => {
                        if (item !== undefined && typeof item.toJSON === 'function') {
                            const serializedItem = item.toJSON();
