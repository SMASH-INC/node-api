...
class ObjectMapper {
    /**
     * Compares input object and the model's own mappingInfo
     * to get addtional properties of the model
     * @param  obj   {object}  The input object containing model properties
     * @param  mappingInfo   {object}  The object containing info about model properties
     *
     * @returns    {array}    An array containing additional properties of the model
     */
    static getAdditonalProperties(obj, mappingInfo) {
        if (obj === null || obj === undefined) return null;
        const props = Object.keys(obj);
        const addProperties = [];
        const mappingInfoArray = (mappingInfo) ? mappingInfo.map(m => m.realName) : [];
        for (let iter = 0; iter < props.length; iter += 1) {
            if (mappingInfoArray.indexOf(props[iter]) === -1) {
                // value from obj not found in mapping info
                addProperties[props[iter]] = obj[props[iter]];
            }
        }
    ...
    mapFields(json, instance) {
        ...
        const props = ObjectMapper.getAdditonalProperties(json, fieldsInfo);
        if (props !== null && props !== undefined) {
            Object.keys(props).forEach((prop) => {
                if (Object.prototype.hasOwnProperty.call(returnInstance, 'additionalProperties')) {
                    returnInstance.addAdditionalProperty(prop, props[prop]);
                }
            });
        }
